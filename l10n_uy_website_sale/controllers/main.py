...
class L10nUYWebsiteSale(WebsiteSale):
    ...
    def _get_mandatory_fields_billing(self, country_id=False):
        ...
        city_id and district fields when company is Uruguay"""
        res = super()._get_mandatory_fields_billing(country_id)
        if request.website.sudo().company_id.country_id.code != "UY":
            return res
        # For Uruguayan company, the VAT is required for all the partners
        res.append("vat")
        if country_id == request.website.sudo().company_id.country_id.id:
            res += ["l10n_latam_identification_type_id"]
            # res.remove("zip")
        return res

    # def _get_mandatory_fields_shipping(self, country_id=False):
    #     """Extend mandatory fields to add city_id and district fields when the selected country is Uruguay"""
