...
class WizardInvoiceSaleCommission(models.Model):
	...
	def print_commission_report(self):
		temp = []
		sale_invoice_commission_ids = self.env['invoice.sale.commission'].sudo().search([('date','>=',self.start_date),('date','<=',self.end_date),('user_id','=',self.salesperson.id)])
...
class AccountInvoice(models.Model):
	...
	commission_ids = fields.One2many('invoice.sale.commission', 'invoice_id', string='Sales Commissions', groups='sales_commission_generic.group_sales_commission_manager',
									 help="Sale Commission related to this invoice(based on sales person)")
	check_refund_commission = fields.Boolean(string="Check Refund",compute="_check_refund")


	def _check_refund(self):
		for rec in self:
			credit_ids = self.env['account.move'].search([('move_type','=','out_refund'),('payment_state','in',['paid','in_payment']),('reversed_entry_id','=',rec.id)])
			if credit_ids: 
				for credit in credit_ids:
					if rec.name == credit.reversed_entry_id.name:
						rec.update({'check_refund_commission': True}) 
						if  rec.check_refund_commission == True:
								for line in rec.commission_ids:
									if line.state == 'to_invoice':
										line.write({
												'state':'cancel'
											})
			else:
				rec.check_refund_commission == False
			...
			for line in invoice.invoice_line_ids:
				if line.is_downpayment == False and line.display_type == 'product':
					...
					if standard_invoice_commission_data and standard_commission_amount > 0:
						commission_line = invoice_commission_obj.sudo().search([('invoice_id', '=', invoice.id),('invoice_line_id', '=', line.id)])
						if not commission_line:
							invoice_commission_ids.append(invoice_commission_obj.sudo().create(standard_invoice_commission_data))
	...
	def get_exceptions(self, commission_brw):
		...
		exception_list = []
		for exception in commission_brw.exception_ids:
			...
			if product_exception_id:
				for prod in product_exception_id:
					if prod not in exception_list:
						exception_list.append(prod)

			subcateg_exception_id = exception_obj.search([
										   ('commission_id', '=', commission_brw.id),
										   ('based_on', '=', 'Product Sub-Categories')])
			if subcateg_exception_id:
				if exception.sub_categ_id.parent_id.id in subcateg_exception_id.category_store.ids:
					for subcateg in subcateg_exception_id:
						if subcateg not in exception_list:
							exception_list.append(subcateg)
			...
			if exclusive_categ_exception_id:
				if exception.categ_id.id in exclusive_categ_exception_id.category_store.ids:
					for categ in exclusive_categ_exception_id:
						if categ not in exception_list:
							exception_list.append(categ)
		...
		if invoice.move_type == 'out_invoice':
			for line in invoice.invoice_line_ids:
				if line.is_downpayment == False and line.display_type == 'product':
					...
					exception_ids = self.get_exceptions(commission_brw)
					margin = ((line.price_subtotal / line.quantity) - line.product_id.standard_price)
					cost = line.product_id.standard_price
					if cost == 0.0:
						actual_margin_percentage = (margin * 100) / 1.0 
					else:
					...
					if invoice_commission_data and commission_amount > 0:
						commission_line = invoice_commission_obj.sudo().search([('invoice_id', '=', invoice.id),('invoice_line_id', '=', line.id)])
						if not commission_line:
							invoice_commission_ids.append(invoice_commission_obj.sudo().create(invoice_commission_data))
		elif invoice.move_type == 'out_refund':  
			for line in invoice.invoice_line_ids:
				if line.is_downpayment == False and line.display_type == 'product':
					amount = line.price_subtotal
	...
	def get_discount_commission(self, commission_brw, invoice):
		...
		for line in invoice.invoice_line_ids:
			if not line.is_downpayment and line.display_type == 'product':
				amount = line.price_subtotal
				invoice_commission_data = {}
				...
				if commission_brw.based_on_ids:
					find_tag = line.product_id.mapped('product_tag_ids').ids
					for rule in commission_brw.based_on_ids:
						if rule.based_on == 'Products':
							commission_line = self.env['discount.commission.basedon'].search(
								[('commission_id', '=', commission_brw.id),
								('discount_percentage', '=', sol_discount),
								('product_id', '=', line.product_id.id)], limit=1)
							if commission_line and commission_line not in discount_list:
								name = f'Commission Type Discount and Based On {rule.based_on} for ({sol_discount} %) discount is ({commission_line.commission_percentage} %) commission'
								discount_list.append(commission_line)
								product_list.append(line.product_id)
								commission_percentage = commission_line.commission_percentage
								flag = False  

						elif rule.based_on == 'Product Categories':
							commission_line = self.env['discount.commission.basedon'].search(
								[('commission_id', '=', commission_brw.id),
								('discount_percentage', '=', sol_discount),
								('categ_id', '=', line.product_id.categ_id.id)], limit=1)
							if commission_line and commission_line not in discount_list:
								name = f'Commission Type Discount and Based On {rule.based_on} for ({sol_discount} %) discount is ({commission_line.commission_percentage} %) commission'
								discount_list.append(commission_line)
								product_list.append(line.product_id)
				...
				if flag:
					...
					if not name:
						name = f'Discount Based commission for ({sol_discount} %) discount is ({commission_percentage} %) commission'
				...
				if invoice_commission_data and commission_amount > 0:
					commission_line = invoice_commission_obj.sudo().search(
			...
			for line in invoice.invoice_line_ids:
				if line.is_downpayment == False and line.display_type == 'product':
					...
					for exception in exception_ids:
						...
						commission_precentage = commission_brw.standard_commission
						name = ''
						if exception.product_id.id == line.product_id.id or exception.categ_id.id == line.product_id.categ_id.id or exception.sub_categ_id.parent_id.id == line.product_id.categ_id.id:
				...
				if payment_state_matters:
					...
					if currency.is_zero(invoice.amount_residual):
						if any(x['has_payment'] or x['has_st_line'] for x in reconciliation_vals):
							...
							if commission_configuration == 'payment' and new_pmt_state in ['paid','in_payment']:
